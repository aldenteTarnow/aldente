...
const batchData = (ids) => {
    ...
    const postMetaData = '?fields=story,message,created_time,description,from,link,full_picture';
    const likesData = 'likes?fields=id&summary=total_count';

    return Promise.all([gatherResources(postMetaData), gatherResources(likesData)]).then((result) => {
...
const getData = () => {
    ...
    return graph.getAsync(url).then(({data}) => {
        ...
        return batchData(ids).then((result) => {
            return graph.getAsync('293169260696126', {fields: 'picture.type(small)'}).then(({picture}) => {
